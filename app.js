...
const itinerary = [
  {day:1,date:"Sat, Jul 18",route:"LAX → LA icons → Ventura",sleep:"Four Points Ventura Harbor Resort",stops:[
    ...
    stop("d1-2","12:15","Erewhon Santa Monica","Hailey Bieber smoothie and quick lunch.","Erewhon Santa Monica",["Kids"]),
    stop("d1-3","1:15","Beverly Hills / Rodeo Drive","Beverly Hills sign, Rodeo Drive, photo stop.","Beverly Hills Sign",["Photo"]),
    stop("d1-4","2:15","Hollywood Walk of Fame","TCL Chinese Theatre and Dolby Theatre.","Hollywood Walk of Fame",["Kids"]),
    stop("d1-5","3:30","Griffith Observatory","Best easy view of the Hollywood Sign.","Griffith Observatory",["Photo"]),
    stop("d1-6","4:30","Drive to Ventura","Expect LA traffic. Do not add Malibu today.","Four Points by Sheraton Ventura Harbor Resort",["Drive"]),
    stop("d1-7","Evening","Check in: Four Points Ventura","Booked direct with Marriott. Confirmation 75176420. Check-in 3:00 PM, checkout noon.","Four Points by Sheraton Ventura Harbor Resort",["Sleep","Booked"])
  ]},
...
const dayBriefs = {
  1:{wake:"Naperville / ORD",sleep:"Ventura",drive:"3.5–5 hrs total",must:"Delta DL1556, National rental, LA icons",leaveBy:"Leave LA by 4:30 PM",dinner:"Ventura Harbor",warning:"Ventura keeps the trip moving north. Do not let LA turn into a late-night drive."},
...
function renderRouteCard(d){
  ...
  return `<article class="card route-card ${open?'open':''}" data-day-card="${d.day}">
    <div class="route-top">
      ...
      <div class="progress-mini">${done}/${d.stops.length}<br/>done</div>
    </div>
...
function renderHighlight(s){
  return `<a class="highlight" href="${mapUrl(s.map)}" target="_blank" rel="noreferrer"><span>${escapeHtml(s.time)}</span><strong>${escapeHtml(s.title)}</strong><em>${escapeHtml(s.desc)}</em></a>`;
}
function renderCheckRow(s){
  ...
  return `<article class="check-row ${done?'done':''}">
    ...
    <div><div class="check-time">${escapeHtml(s.time)}</div><h4>${escapeHtml(s.title)}</h4><p>${escapeHtml(s.desc)}</p><div class="tag-list">${s.tags.map(t=>`<span class="tag ${tagClass(t)}">${escapeHtml(t)}</span>`).join('')}</div></div>
